...
resource "google_artifact_registry_repository_iam_member" "this" {
  ...
  repository = "projects/${var.gar_repositories[0].project_id != null ? var.gar_repositories[0].project_id : local.google_project_id}/locations/${var.gar_repositories[0].region}/repositories/${length(split("repositories/", each.value[0].name)) == 2 ? split("/", split("repositories/", each.value[0].name)[1])[0] : each.value[0].name}"
