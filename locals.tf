  ...
  gar_repositories = flatten([
    for gar in var.gar_repositories : [
      for repo in gar.repositories : merge(repo,
        {
          repo           = "${gar.region}-docker.pkg.dev/${gar.project_id != null ? gar.project_id : local.google_project_id}/${length(split("repositories/", repo.name)) == 2 ? split("/", split("repositories/", repo.name)[1])[0] : repo.name}"
